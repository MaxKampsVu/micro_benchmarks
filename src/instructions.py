def write_leaky_isa(reg_ow_flag, sram_ow_flag, remnant_flag, leaky_isa_file):
    with open(leaky_isa_file, 'w') as f:
        if remnant_flag:
            f.write(gen_remnant_leak() + '\n')
        
        f.write(gen_xor(reg_ow_flag) + '\n')
        f.write(gen_and(reg_ow_flag) + '\n')
        f.write(gen_nop() + '\n')
